      ...
      >
        ...
        <div class="choice">
          <van-radio-group v-model="item.isDefault">
            <van-radio name="1" checked-color="#07c160" @click="radiol(item.id)">设为默认</van-radio>
          </van-radio-group>
          <div class="editor">
...
export default {
  ...
  methods: {
    qieaddress(i) {
      console.log(i);
      localStorage.setItem("add", JSON.stringify(i.id));
      this.$router.push({
        name: "order",
        params: {
          addresslist: i,
          ...
          if (res.data.result == "0") {
            ...
            for (let i in this.dataList[0]) {
              if (this.dataList[0][i].isDefault == "1") {
                this.radio = this.dataList[0][i].id;
